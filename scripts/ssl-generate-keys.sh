...
KUBERNETES_SVC=10.103.0.1 NODE_IP=172.16.22.10 openssl req -new -key ${SCRIPT_ROOT}/kubernetes-key.pem -out ${SCRIPT_ROOT}/kubernetes.csr -subj "/CN=kubernetes" -config ${SCRIPT_ROOT}/master-openssl.cnf
KUBERNETES_SVC=10.103.0.1 NODE_IP=172.16.22.10 openssl x509 -req -in ${SCRIPT_ROOT}/kubernetes.csr -CA ${SCRIPT_ROOT}/ca.pem -CAkey ${SCRIPT_ROOT}/ca-key.pem -CAcreateserial -out ${SCRIPT_ROOT}/kubernetes.pem -days 365 -extensions v3_req -extfile ${SCRIPT_ROOT}/master-openssl.cnf
